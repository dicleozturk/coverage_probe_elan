...
def failure_flag(meta):
    ...
    # A model id the provider does not recognise: the name in the preset has
    # gone stale. Worth telling apart from a call that failed for other reasons.
    if "not a valid model ID" in err or "invalid model" in err.lower():
        return "NOT_RUN(model_unavailable)"
    # No credit on the account. Says nothing about the model or the language.
    if "402" in err or "Insufficient credits" in err:
        return "NOT_RUN(no_credits)"
